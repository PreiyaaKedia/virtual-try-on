...
if ($containerAppExists) {
    ...
    if ($useKeyVault -eq "y") {
        ...
        az keyvault set-policy --name $keyVaultName --resource-group $resourceGroupName --object-id $principalId --secret-permissions get list
    ...
    if ($useKeyVault -eq "y") {
        ...
        $createCmd += " --assign-identity system"
    ...
    # If we created the container app with Key Vault integration, we need to grant the app permission to access Key Vault
    if ($useKeyVault -eq "y") {
        # Get the principal ID of the container app's managed identity
        $principalId = az containerapp identity show --name $containerAppName --resource-group $resourceGroupName --query "principalId" -o tsv
